...
public class MemberRepositoryImpl implements MemberRepositoryCustom {
  ...
  public MemberAuthorityProvider.RoleDto fetchRoleByStudentId(Long id) {
    Role role =
        queryFactory
            .select(member.ibasInformation.role)
            .from(member)
  ...
  public boolean isDuplicated(MemberDuplicationQueryCondition condition) {
    ...
    return !queryFactory.selectFrom(member).where(eqAny(condition)).limit(1).fetch().isEmpty();
  }
  ...
  private BooleanBuilder eqAny(MemberDuplicationQueryCondition condition) {
    ...
    return booleanBuilder.or(eqProvider(condition.getProvider())).or(eqUid(condition.getUid()));
  }
