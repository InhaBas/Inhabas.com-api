...
public class JwtTokenUtilTest {
  ...
  public void createJwtTokenTest() {
    ...
    assertThat(accessToken.chars().filter(ch -> ch == '.').count()).isEqualTo(2);
  }
  ...
  public void reissueAccessToken() {
    ...
    JwtAuthenticationToken authenticateNewAccessToken =
        jwtTokenUtil.getAuthentication(newAccessToken);
  ...
  public void validateInvalidToken() {
    ...
    assertThrows(InvalidTokenException.class, () -> jwtTokenUtil.validate("invalid-token-string"));
  }
  ...
  public void validateValidToken() {
    ...
    List<SimpleGrantedAuthority> authorities =
        List.of(
            new SimpleGrantedAuthority("ROLE_ANONYMOUS"), new SimpleGrantedAuthority("TEAM_IT"));
    Map<String, Object> attributes =
        new HashMap<>() {
          {
            put("sub", "1249846925629348");
