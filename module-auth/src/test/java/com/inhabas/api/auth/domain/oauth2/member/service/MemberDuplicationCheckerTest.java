...
public class MemberDuplicationCheckerTest {
  ...
  @InjectMocks MemberDuplicationCheckerImpl memberDuplicationChecker;
  ...
  public void notDuplicatedUidTest() {
    // given
    given(memberRepository.isDuplicated(any(MemberDuplicationQueryCondition.class)))
        .willReturn(false);

    // when
    Assertions.assertFalse(
        memberDuplicationChecker.isDuplicatedMember(
            new MemberDuplicationQueryCondition(OAuth2Provider.GOOGLE, "12312312312312")));
    then(memberRepository)
        .should(times(1))
        .isDuplicated(any(MemberDuplicationQueryCondition.class));
  }
