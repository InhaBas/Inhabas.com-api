...
public class SignUpIntegrationTest {
  ...
  public void 회원가입_기간이_아닙니다() throws Exception {
    ...
    String response =
        mockMvc
            .perform(get("/signUp").with(accessToken(token)))
            .andExpect(status().isForbidden())
            .andReturn()
            .getResponse()
            .getContentAsString(StandardCharsets.UTF_8);
    assertThat(response)
        .isEqualTo("{\"status\":403,\"code\":\"S001\",\"message\":\"회원가입 기간이 아닙니다.\"}");
  }
  ...
  public void OAuth2_인증_후_비회원_신규_학생_회원가입() throws Exception {
    ...
    소셜 로그인 후 회원 가입용 임시 토큰을 발급 받았다.*/
    mockMvc
        .perform(get("/signUp/check").with(accessToken(token)))
    ...
    mockMvc
        ...
        .andExpect(
            content()
                .string(
                    "{\"name\":null,\"major\":null,\"phoneNumber\":null,\"studentId\":null,\"memberType\":null,\"grade\":null}"));

    /* 개인정보 입력을 위해, 전공 정보들이 로딩된다. */
    mockMvc.perform(get("/signUp/majorInfo").with(accessToken(token))).andExpect(status().isOk());
    ...
    String questionList =
        mockMvc
            .perform(get("/signUp/questionnaires").with(accessToken(token)))
            .andExpect(status().isOk())
            .andReturn()
            .getResponse()
            .getContentAsString(StandardCharsets.UTF_8);
    ...
    mockMvc
        .perform(get("/signUp/answers").with(accessToken(token)))
        .andExpect(status().isOk())
        .andExpect(
            content()
                .string(
                    "[{\"questionId\":1,\"content\":null},{\"questionId\":2,\"content\":null},{\"questionId\":3,\"content\":null},{\"questionId\":4,\"content\":null},{\"questionId\":5,\"content\":null}]"));
    ...
    Member 유동현 =
        memberRepository
            .findByProviderAndUid(
                OAuth2Provider.NAVER, new UID("N8ojJQXxxSxtO0CmEH3xtt5Y6ER09UEsRozkpbGAdOI"))
            .orElseThrow();
  ...
  public void OAuth2_인증_후_비회원_신규_교수_회원가입() throws Exception {
    ...
    String majorList =
        mockMvc
            .perform(get("/signUp/majorInfo").with(accessToken(token)))
            .andExpect(status().isOk())
            .andReturn()
            .getResponse()
            .getContentAsString(StandardCharsets.UTF_8);
    assertThat(majorList)
        .isEqualTo(
            "["
    ...
    /* 회원가입 신청을 완료한다. */
    mockMvc.perform(put("/signUp").with(accessToken(token))).andExpect(status().isNoContent());
  ...
  private void forbiddenWhenAccessEverySignUpApi(Role role) throws Exception {
    ...
    mockMvc.perform(get("/signUp").with(accessToken(token))).andExpect(status().isForbidden());
    mockMvc.perform(post("/signUp").with(accessToken(token))).andExpect(status().isForbidden());
    mockMvc
        .perform(get("/signUp/answers").with(accessToken(token)))
        .andExpect(status().isForbidden());
    mockMvc
        .perform(post("/signUp/answers").with(accessToken(token)))
        .andExpect(status().isForbidden());
  }
  ...
  private String OAuth인증된_JWT_TOKEN(Role role) {
    List<? extends GrantedAuthority> grantedAuthorities =
        List.of(new SimpleGrantedAuthority(ROLE_PREFIX + role.toString()));
    Map<String, Object> nameAttributeKey =
        Map.of(
            "message",
            "success",
            "response",
