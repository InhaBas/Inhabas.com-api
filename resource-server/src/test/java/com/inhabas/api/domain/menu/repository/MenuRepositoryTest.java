...
public class MenuRepositoryTest {
  ...
  public void CreateNewMenu() {
    ...
    Menu activityBoardMenu =
        new Menu(menuGroup1, 1, MenuType.LIST, "동아리 활동", "동아리원의 활동을 기록하는 게시판입니다.");
  ...
  public void CannotSamePriorityValue() {
    ...
    Menu freeBoardMenu =
        new Menu(menuGroup2, 2, MenuType.LIST, "자유게시판", "부원이 자유롭게 글을 작성할 수 있는 게시판입니다.");
    ...
    assertThrows(
        ...
        () ->
            menuRepository.save(
                new Menu(menuGroup2, 2, MenuType.LIST, "질문게시판", "궁금한 점을 질문하는 게시판입니다.")));
  }
  ...
  public void findAllMenuGroupByMenuGroup() {
    ...
    List<Menu> menuList =
        Arrays.asList(
            menu11, menu12, menu13, menu14, menu21, menu22, menu23, menu24, menu31, menu32, menu41,
            menu42, menu51);
    ...
    // then
    MenuGroupDto menuGroupDto1 =
        new MenuGroupDto(
            menuGroup1.getId(),
            menuGroup1.getName(),
            Arrays.asList(
                MenuDto.convert(menuList.get(0)),
                MenuDto.convert(menuList.get(1)),
                MenuDto.convert(menuList.get(2)),
                MenuDto.convert(menuList.get(3))));
    MenuGroupDto menuGroupDto2 =
        new MenuGroupDto(
            menuGroup2.getId(),
            menuGroup2.getName(),
            Arrays.asList(
                MenuDto.convert(menuList.get(4)),
                MenuDto.convert(menuList.get(5)),
                MenuDto.convert(menuList.get(6)),
                MenuDto.convert(menuList.get(7))));
    MenuGroupDto menuGroupDto3 =
        new MenuGroupDto(
            menuGroup3.getId(),
            menuGroup3.getName(),
            Arrays.asList(MenuDto.convert(menuList.get(8)), MenuDto.convert(menuList.get(9))));
    MenuGroupDto menuGroupDto4 =
        new MenuGroupDto(
            menuGroup4.getId(),
            menuGroup4.getName(),
            Arrays.asList(MenuDto.convert(menuList.get(10)), MenuDto.convert(menuList.get(11))));
    MenuGroupDto menuGroupDto5 =
        new MenuGroupDto(
            menuGroup5.getId(), menuGroup5.getName(), List.of(MenuDto.convert(menuList.get(12))));
    List<MenuGroupDto> expectedMenuGroupDtoList =
        Arrays.asList(menuGroupDto1, menuGroupDto2, menuGroupDto3, menuGroupDto4, menuGroupDto5);

    assertThat(allMenuInfo).usingRecursiveComparison().isEqualTo(expectedMenuGroupDtoList);
  }
}
