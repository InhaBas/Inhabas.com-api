...
public class PolicyTermControllerTest {
    ...
    @MockBean
    private PolicyTermService policyTermService;
    ...
    void findPolicyTerm() throws Exception {
        ...
        //when
        String response = mvc.perform(get("/policy/{policyTermId}", 1L))
    ...
    void findPolicyTerm_Invalid_Input() throws Exception {
        ...
        //when
        String response = mvc.perform(get("/policy/{policyTermId}", "invalid"))
    ...
    void updatePolicyTerm() throws Exception {
        //given
        SavePolicyTernDto savePolicyTernDto = SavePolicyTernDto.builder()
                .content("meaningless")
                .build();
        doNothing().when(policyTermService).updatePolicyTerm(any(), any());

        //when then
        mvc.perform(put("/policy/{policyTermId}", 1L)
    ...
    void updatePolicyTerm_Invalid_Input() throws Exception {
        ...
        //when
        String response = mvc.perform(put("/policy/{policyTermId}", 1L)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(jsonOf(savePolicyTernDto)))
                .andExpect(status().isBadRequest())
                .andReturn()
