...
class MyProfileControllerTest {
  ...
  public void updateMyProfileImage() throws Exception {
    //given
    MockMultipartFile file = new MockMultipartFile("picture", "test.jpeg", "image/jpeg", "<<jpeg data>>".getBytes());

    willDoNothing().given(memberProfileService).updateMyProfileImage(any(), any());

    //when then
    mvc.perform(multipart("/myInfo/picture")
            .file(file))
            .andExpect(status().isNoContent());
  ...
  public void getMyInfoMyRequestsTest() throws Exception {
    //given
    UpdateNameRequestDto dto = UpdateNameRequestDto.builder()
            .id(1L)
            .major("컴퓨터공학과")
            .role(Role.BASIC)
  ...
  public void handleMyInfoRequestTest() throws Exception {
    // given
    HandleNameRequestDto handleNameRequestDto = HandleNameRequestDto.builder()
            .id(1L)
            .status("pass")
            .build();
    ...
    //when then
    mvc.perform(put("/myInfo/request")
            .contentType(MediaType.APPLICATION_JSON)
