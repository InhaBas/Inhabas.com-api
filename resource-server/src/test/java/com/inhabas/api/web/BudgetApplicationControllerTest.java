...
public class BudgetApplicationControllerTest {
  ...
  @MockBean private BudgetApplicationService service;
  @MockBean private BudgetApplicationProcessor processor;
  ...
  //                        .content("{\"title\":\"서버 사용비\", \"dateUsed\":\"2000-01-01T00:00:00\",
  // \"details\":\"aws 크롤링 비용\", \"outcome\":50000, \"accounts\":\"카카오 01-00022-13204 유동현\"}"))
  ...
  //                        .contentType(MediaType.APPLICATION_JSON)
  //                        .content("{\"id\":1,\"title\":\"서버 사용비\",
  // \"dateUsed\":\"2000-01-01T00:00:00\", \"details\":\"aws 크롤링 비용\", \"outcome\":50000,
  // \"accounts\":\"카카오 01-00022-13204 유동현\", \"applicationId\":1}"))
  //                .andExpect(status().isNoContent());
  //
  //        //then
  //        then(service).should(times(1))
  ...
  public void getApplicationTest() throws Exception {
    ...
    mockMvc.perform(get("/budget/application/1")).andExpect(status().isOk());

    // then
    then(service).should(times(1)).getApplicationDetails(anyInt());
  }
  ...
  public void searchApplicationTest() throws Exception {
    ...
    mockMvc.perform(get("/budget/applications")).andExpect(status().isOk());
