...
public class BudgetApplicationRepositoryImpl implements BudgetApplicationRepositoryCustom {
        ...
        queryFactory
            .select(
                Projections.constructor(
                    BudgetApplicationListDto.class,
            ...
            .innerJoin(applicant)
            .on(budgetSupportApplication.applicationWriter.eq(applicant.studentId))
            .where(
                sameStatus(status)
                    .and(budgetSupportApplication.status.ne(ApplicationStatus.PROCESSED)))
  ...
  private BooleanExpression sameStatus(ApplicationStatus status) {
    ...
    return status == null
        ? budgetSupportApplication.status.ne(ApplicationStatus.PROCESSED)
    ...
    return queryFactory
        ...
        .on(
            budgetSupportApplication.personInCharge.eq(
                pic.studentId)); // 총무가 아직 승인 또는 거절 안했을수 있기 때문에 null 일 수 있다.
  }
}
