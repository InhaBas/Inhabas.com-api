...
public class CommentController {
    ...
    @Operation(summary = "해당 게시글의 댓글을 반환한다.",
            description = "댓글에 대한 조회 권한은 해당 게시판의 조회 권한을 따른다. ")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", content = {@Content(
                    schema = @Schema(implementation = CommentDetailDto.class))}),
    ...
    @PreAuthorize("@boardSecurityChecker.checkMenuAccess(#menuId, T(com.inhabas.api.domain.board.usecase.BoardSecurityChecker).READ_COMMENT)")
    ...
    @Operation(summary = "댓글을 생성하기 위한 요청을 한다.",
            description = "parent_comment_id 값이 주어지면 대댓글, 아무값도 없으면 그냥 댓글 / 댓글에 대한 생성 권한은 해당 게시판의 생성 권한을 따른다. ")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", content = {@Content(
                    schema = @Schema(implementation = CommentDetailDto.class))}),
            @ApiResponse(responseCode = "400 ", description = "입력값이 없거나, 타입이 유효하지 않습니다.", content = @Content(
    ...
    @PreAuthorize("@boardSecurityChecker.checkMenuAccess(#menuId, T(com.inhabas.api.domain.board.usecase.BoardSecurityChecker).CREATE_COMMENT)")
    ...
    @Operation(summary = "댓글을 수정하기 위한 요청을 한다.",
            description = "작성자, 부회장 이상 권한을 가진 유저만 가능")
    @ApiResponses(value = {
