...
public class BudgetApplicationController {
  ...
  public ResponseEntity<?> createApplication(
      @Authenticated StudentId loginMember,
  ...
  @ApiResponse(
      responseCode = "200",
      description = "검색 결과를 pagination 적용해서 반환, status 값 안주면 전체 상태를 검색함.")
  public ResponseEntity<Page<BudgetApplicationListDto>> getApplications(
      @Nullable @RequestParam ApplicationStatus status,
      @PageableDefault(size = 15, sort = "dateUsed", direction = Sort.Direction.DESC)
          Pageable pageable) {

    Page<BudgetApplicationListDto> page =
        budgetApplicationService.getApplications(status, pageable);
